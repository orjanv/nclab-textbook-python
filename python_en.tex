...
using the modulo operator, as will be shown in the next paragraph. But before we 
move there, let's summarize:\\

\begin{gbox}
\begin{center}
{Failure to convert at least one operand in division 
to a real number can be\\ a source of mistakes that are extremely hard to find.}
\end{center}
\end{gbox}
...
Python respects the standard priority of operators:
...
\begin{itemize}
\item Review and refine your knowledge of custom functions.
...
but in Python they can moreover accept {\em input arguments}. Recall that 
...
The definition of a new function begins with the keyword {\tt def} as in Karel. In Python,
...
\begin{gbox}
\noindent
{\bf It is a good programming practice to}:
\begin{itemize}
\item {\em Have names of variables reflect their contents}. 
      ...
      becomes unreadable.
\item {\em Avoid abuse of dynamic type interpretation}. A variable, once
...
{\bf It is a good programming practice to:}
\begin{itemize}
\item {\em Avoid global variables in functions}. 
...
algorithm is very simple: We will generate $n$ random points inside 
the rectangle, and count how many of them fall inside the ellipse.
...
the counting loop in Python. Recall that to make Karel

...
In NCLab's Python module, Python code is typed into one or more {\em code cells}. 
...
\subsection{Downloading Python programs from NCLab's database}

NCLab's public database can be accessed from the File manager's File 
menu or from the File menu of the Python module. In the Search box 
that appears, enter the keywords that best match what you are looking 
for. For example, typing "textbook python" will find exercises that 
belong to the Python textbook. Any project that you find you can download
into your account, modify, and use in any way you wish.

\subsection{Launching the Python module}

The Python module can be launched via 
the Programming icon on Desktop or via File manager's File menu. Initially,
the module will contain a {\em demo program} -- Python program that can be 
run instantly by pressing the green arrow button. The demo program is not 
necessarily the simplest possible Python program. Its purpose is to illustrate 
the usage of the Python module and yield interesting graphics. The output 
of the present demo program is the so-called {\em Dragon fractal},
as shown in Fig. \ref{fig:python}.

After you do  not need the demo program anymore, you can turn it off in 
Settings. Next time you launch the Python module, it will not contain any 
code.
\newpage
...
primary colors in the mix. It is customary to define these proportions by an integer number 
between 0 and 255 for each primary color. So, the resulting color is a triplet of 
integers between 0 and 255. For the primary colors we have\\[-4mm]
...
\end{center}
\vspace{0mm}
These colors are shown in the following Fig. \ref{fig:rgb}.\\[-7mm]
...
\end{center}
\vspace{-2mm}
\caption{Left: pure red color [255, 0, 0]. Middle: pure green color [0, 255, 0]. Right; pure blue color [0, 0, 255].}
\label{fig:rgb}
\end{figure}
\noindent
...
\end{center}
\vspace{-2mm}
\caption{Left: black color [0, 0, 0]. Middle: dark grey color [128, 128, 128]. Right; light grey color [220, 220, 220].}
\vspace{-1cm}
\label{fig:rgb2}
\end{figure}
\newpage
...
colors into RGB codes. Fig. \ref{fig:rgb3} shows three "simple" colors
...
\end{center}
\vspace{-2mm}
\caption{Left: cyan color [0, 255, 255]. Middle: pink color [255, 0, 255]. Right; yellow color [255, 255, 0].}
\label{fig:rgb3}
\end{figure}
...
\noindent
The commands {\tt clf()}, {\tt plot()} and {\tt show()} do clear the canvas, 
...
function. For illustration, changing the fifth line in the last script to {\tt plot(x, y, color = [255, 0, 255])},
...
\PY{n}{color} \PY{o}{=} \PY{p}{[}\PY{l+m+mf}{220}\PY{p}{,} \PY{l+m+mf}{220}\PY{p}{,} \PY{l+m+mf}{220}\PY{p}{]}  \PY{c}{\PYZsh{} This is light-grey color.}
...
which are between 0 (darkest) and 255 (lightest). The parameter {\tt pad} defines padding.
...
\PY{k}{print} \PY{n}{a}
\PY{n}{a} \PY{o}{=} \PY{l+s}{'}\PY{l+s}{Hello!}\PY{l+s}{'}
\PY{k}{print} \PY{n}{a}
...
\subsection{Overriding constants}

In Python, constants such as {\tt pi} or {\tt e} are in fact just global variables
and in principle there is no problem redefining their values in our program.
Of course this is not a good programming practice as our code becomes
confusing. A variable {\tt pi} should contain the value of $\pi$
as everyone expects. 

But, mistakes happen. 
If we define a function where we create a local variable {\tt pi},
this is in fact another example of variable shadowing and our code 
will work fine:\\

\begin{bbox}
\begin{Verbatim}[commandchars=\\\{\}]
\PY{k+kn}{from} \PY{n+nn}{numpy} \PY{k+kn}{import} \PY{n}{sqrt}\PY{p}{,} \PY{n}{pi}
\PY{k}{def} \PY{n+nf}{hypotenuse}\PY{p}{(}\PY{n}{x}\PY{px}{,} \PY{n}{y}\PY{p}{)}\PY{p}{:}
    pi = \PY{n}{sqrt}\PY{p}{(}\PY{n}{x}\PY{o}{*}\PY{o}{*}\PY{l+m+mi}{2} \PY{o}{+} \PY{n}{y}\PY{o}{*}\PY{o}{*}\PY{l+m+mi}{2}\PY{p}{)}
    \PY{k}{return} pi
\PY{k}{print} \PY{n}{hypotenuse}\PY{p}{(}\PY{l+m+mi}{3}\PY{p}{,} \PY{l+m+mi}{4}\PY{p}{)}
\PY{k}{print} \PY{n}{pi}
\end{Verbatim}
\end{bbox}
\vspace{6mm}
...
Python has a built-in data type, {\tt bool}, for expressing True and False values.  This is short for Boolean. Certain expressions in Python, those that use {\em comparison operators}, evaluate to {\tt True} or {\tt False}. These expressions are used to determine the relationship between two values.\\
\\
...
\noindent
In the above example, {\tt >} is a comparison operator. It is used to evaluate whether the value on the left is greater than the value on the right.\\
\\
Although the syntax looks a bit unusual, this is a correct
Python code -- since {\tt a} is equal to {\tt 1}, the Boolean expression {\tt a > 0}
is {\tt True}. This is the value that was printed by the interpreter.\\
\\
...
Similar to numbers and strings, Boolean values can be stored in variables.
For example:\\
...
\subsection{Monte Carlo methods}\label{subsec:montecarlo}

Monte Carlo methods are popular methods of scientific computing. 
They employ large numbers of random values to calculate approximately
results that are difficult or impossible to obtain exactly. Sounds 
complicated? 
On the contrary, Monte Carlo methods are very simple! For demonstration, 
let us calculate the area of an ellipse. 
...
5 ounces, box B for apples that weight at least five but less than 10 ounces, 
box C for apples that weight at least 10 but less than 15 ounces, etc. Apples weighting 
20 ounces or more will all be put into box E. We need to write a function 
...
functions. Moreover note the mandatory colon '{\tt :}' after the Boolean expression.
...
\subsection{Abusing the {\tt while} loop}
...
\begin{Verbatim}[commandchars=\\\{\}]
\PY{n}{counter} \PY{o}{=} \PY{l+m+mi}{0}
\PY{k}{while} \PY{n}{counter} \PY{o}{<} \PY{l+m+mi}{20}\PY{p}{:}
    \PY{c}{\PYZsh{} Do something with the counter, here}
    \PY{c}{\PYZsh{} for simplicity we just print it:}
    \PY{k}{print} \PY{n}{counter}
    \PY{c}{\PYZsh{} Increase the counter:}
    \PY{n}{counter} \PY{o}{+}\PY{o}{=} \PY{l+m+mi}{1}
\end{Verbatim}
\end{bbox}
\vspace{6mm}
...
\noindent
Although there is nothing syntactically wrong with this code, and it 
even produces the desired result, it is an example of bad 
programming. The person who wrote such a code probably did 
not understand why programming languages have two types 
of loops. The code emulates a counting loop and whoever 
reads it, will be confused. Why was the {\tt while} loop used
when the number of repetitions was known in advance? The correct 
code for this situation is:\\

\begin{bbox}
\begin{Verbatim}[commandchars=\\\{\}]
\PY{k}{for} \PY{n}{counter} \PY{o+ow}{in} \PY{n+nb}{range}\PY{p}{(}\PY{l+m+mi}{20}\PY{p}{)}\PY{p}{:}
    \PY{k}{print} \PY{n}{counter}
\end{Verbatim}
\end{bbox}
...
\subsection{Next trip into scientific computing}
...
\section{More on Counting Loop} \label{sec:forloop}
...
\PY{k}{print} \PY{l+s}{"}\PY{l+s}{Area and perimeter of circle 1:}\PY{l+s}{"}\PY{p}{,} \PYZbs{}
...
\PY{k}{print} \PY{l+s}{"}\PY{l+s}{Area and perimeter of circle 2:}\PY{l+s}{"}\PY{p}{,} \PYZbs{}
...
\PY{k}{print} \PY{l+s}{"}\PY{l+s}{Area and perimeter of circle 3:}\PY{l+s}{"}\PY{p}{,} \PYZbs{}
